...
__all__ = ['init_intrin', 'init_extrin', 'SSE', 'single_calib', 'calib_multi']
...
def calib_multi(df_img,
    ...
    for idx_cam in np.sort(df_img.idx_cam.unique()):
        imgs_cam = df_img[df_img.idx_cam == idx_cam].img.to_list()
        cam, distort, rigids, (pss_c_p, _) = single_calib(imgs_cam,
                                                          ...
                                                          cutoff_it,
                                                          cutoff_norm)
        df_img.loc[df_img[df_img.idx_cam == idx_cam].index, 'ps_c_p'] = pss_c_p
    ...
    # Disable gradient computation for root camera
    ...
    def _get_loss():
        ...
        for _, row in df_img.iterrows():
            m_w2p, ps_c_p = row.m_w2p, row.ps_c_p
            idx = torch.where(torch.all(torch.isfinite(ps_c_p), dim=1))[0]
            ls.append(loss(ps_c_p[idx], m_w2p(ps_c_w[idx])))
        return sum(ls)
    ...
    return (cams, distorts, rigids_pos, rigids_cam,
            torch2np((tuple([row.ps_c_p for _,row in df_img.iterrows()]),
                      tuple([row.m_w2p(ps_c_w) for _,row in df_img.iterrows()]))))