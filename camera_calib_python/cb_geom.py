...
__all__ = ['crrgrid', 'csrgrid', 'cfpgrid', 'csdgrid', 'PGeom', 'BGeom', 'CpGeom', 'CpCSRGrid', 'CpCSDGrid', 'FmGeom',
           'FmCFPGrid', 'CbGeom']
...
#Cell
class CbGeom():
    def __init__(self, h_cb, w_cb, cp_geom, fm_geom=None, idx_remove=np.empty(0, dtype=np.int)):
        self.h_cb, self.w_cb = h_cb, w_cb
        self.cp_geom = cp_geom
        self.fm_geom = fm_geom
        self.idx_remove = idx_remove

    @property
    def ps_c(self): return np.delete(self.cp_geom.ps, self.idx_remove, axis=0)

    @property
    def bs_c(self): return np.delete(self.cp_geom.bs, self.idx_remove, axis=0)

    @property
    def ps_f(self): return self.fm_geom.ps
